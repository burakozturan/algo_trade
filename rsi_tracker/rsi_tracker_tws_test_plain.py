...
EXCHANGE       = "SMART"
CURRENCY       = "USD"
TICK_BY_TICK_MAX = 5   # IB hard-limits simultaneous tick-by-tick streams
# ─────────────────────────────────────────────
...
def subscribe_realtime(ib: IB):
    ...
    def make_tick_handler(sym):
        def on_tick_change(ticker_obj, *_):
            # ib_insync passes the full Ticker object to updateEvent,
            # not a bare list of ticks.
            if hasattr(ticker_obj, "tickByTicks"):
                tbt = ticker_obj.tickByTicks
                price = tbt[-1].price if tbt else getattr(ticker_obj, "last", None)
            elif isinstance(ticker_obj, (list, tuple)):
                if not ticker_obj:
                    return
                price = getattr(ticker_obj[-1], "price", None)
            else:
                return
            apply_intrabar_price(sym, price)
    ...
    for ticker in STOCKS:
        ...
        use_tbt = (ticker in tick_by_tick_symbols
                   and tbt_count < TICK_BY_TICK_MAX)
        if use_tbt:
            ...
            if ticker in tick_by_tick_symbols:
                console.print(f"  [yellow]MktData (tbt cap): {ticker}[/]")
            else:
